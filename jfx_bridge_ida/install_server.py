""" Handle installing the jfx_bridge_ida server scripts (and supporting jfx_bridge) to a specified directory """
import argparse
import os
...
def do_install(install_dir):
    ...
    server_files = [f for f in pkg_resources.resource_listdir(IDA_BRIDGE, SERVER_DIR) if f not in ["__init__.py", "__pycache__"]]
    ...
    for f in server_files:
        ...
        with pkg_resources.resource_stream(IDA_BRIDGE, SERVER_DIR + "/" + f) as resource:
            with open(dest_path, "wb") as dest:
                print("\t" + dest_path)
                dest.write(resource.read())
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Install jfx_bridge_ida server scripts")
